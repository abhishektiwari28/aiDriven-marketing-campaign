...
const AnalyticsFunnel = ({ selectedCampaign, timeRange }) => {
    ...
    return (
        <div className="glass-card p-16 border border-slate-200 flex flex-col items-center justify-center text-center animate-in zoom-in-95 duration-500 min-h-[500px]">
            <div className="w-24 h-24 bg-indigo-50 border border-indigo-100 rounded-3xl flex items-center justify-center mb-10 shadow-inner">
                <GitBranch className="w-12 h-12 text-indigo-600" />
            </div>
            <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight">Deployment Pipeline</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 w-full max-w-2xl">
                {steps.map((step, i) => (
                    <div key={i} className="p-6 rounded-2xl bg-white border border-slate-100 shadow-sm hover:border-indigo-200 transition-all">
                        <p className="text-[10px] text-slate-400 font-black uppercase mb-1.5 tracking-widest">{step.label}</p>
